...
#from sklearn.cluster import DBSCAN #k-meansより合理的にクラスタを作るやつ
#from sklearn.decomposition import TruncatedSVD #t-SNEより軽いけど，結果はよくない
#from sklearn.manifold import TSNE #t-SNEのsklearn実装，使いづらすぎる
import scipy.fftpack
from scipy import signal

#SSNMFを行う際によく使う関数をクラス化
class Functions():
    def __init__(self, basis, iter, ss_basis, ss_iter, sr=44100):
     ...
     # スペクトログラム を返す関数
    def stft(self, filename, Normalize = None):
        s, sr = librosa.load(filename + ".wav")
        self.sr = sr
        if Normalize != None:
            s = np.pad(s,(0,Normalize-len(s)),"constant")
        stft_wav = librosa.stft(s)#s[0:len(s)*10])
    ...
    def train_vvvf(self, stft, R=30, init_F=None, init_G=None, init='random', beta=2):
        ...
        return NMF(stft, R=R, n_iter=self.iter, init_F=init_F, init_G=init_G, init=init, beta=beta, verbose=False)
    ...
    def remove_noise(self, stft, tsne_result, kmeans_tsne, RemNum):
        #RemNum = 2,5 #6000系の場合は3と6がゴミクラスタだった
        print(stft.shape)
        rem_list = np.array([])
        for i in RemNum:
            i = i - 1
    ...
    def plot_amptitude_frequency(self, ssnmf):
        A = a[1].sum(axis=0)
        N = len(A)
        win = signal.hann(N)
        spectrum = scipy.fftpack.fft(A * win)
        plt.plot(np.abs(spectrum[0:15]))

     # 探索したperplexityで計算したt-sneの結果を保存(学習に時間がかかるので)
    def save_tsne_result(self, cepstrums, fname, perNum=160):
    ...
    #nmfが得る左基底行列の基底が調波構造を持つか調べる
    def nmf_coefficient(self, cepstrums, nmf_cepstrums, clusta_leader):
        cp = clusta_leader
    ...
    #2つの音声に対し導出した各クラスタリーダーのケプストラムを
    #全組み合わせ分相関係数を算出し（xcorrは谷と山でも強調されるからダメ），
    #ある閾値を超えたものだけ残す
    def clusta_coefficient(self, ceps1, ceps2, clusta_leader1, clusta_leader2, Threshold, plot_figure = False):
        len1 = len(clusta_leader1)
        len2 = len(clusta_leader2)
        survive_clusta = []
        if len1 != len2:
            raise ValueError("You need mutching the lentgh")
        for i in range(len1):
            c1 = ceps1.T[:, clusta_leader1[i]]
            for j in range(len2):
                c2 = ceps2.T[:, clusta_leader2[j]]
                coef = np.corrcoef(c1,c2)[0][1]
                if plot_figure == True:
                    plt.figure()
                    plt.subplot(3,1,1)
                    plt.plot(c1)
                    plt.title("cepstrum1 : clusta No.{}".format(i))
                    plt.subplot(Num*3,3,2)
                    plt.plot(c2)
                    plt.title("cepstrum2 : clusta No.{}".format(j))
                    plt.subplot(Num*3,3,3)
                    plt.plot(c1,c2,color="r")
                    plt.title("corrcoef : {}".format(coef))
                    plt.show()
                if coef >= Threshold:
                   survive_clusta.append([i,j,coef])
        return survive_clusta
