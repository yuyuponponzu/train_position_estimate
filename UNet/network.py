...
def Augmentation_4(y_inv, y_noi, X, Y, idx_item, i):
    cnt = 0
    #一対のデータに対して2*(2*2+2*1)+4=16個のデータを生成
    st_ = [1.0, 0.75]
    for sh in range(0,2):
        y_noi_shift = pitch_shift(y_noi, const.SR, sh)
        X, Y, cnt = Savespec(y_inv, y_noi_shift, X , Y, idx_item, i, cnt, ratio=st_)
...
def Augmentation_main(inverter_, fl_noise, X, Y, idx_item, i):
    ...
    return Augmentation_4(y_inverter, y_noise, X, Y, idx_item, i)

def TrainUNet(PATH_inverter = "Noise", PATH_noise = "Inverter", epoch=250, savefile="unet.model"):
    unet = UNet()
    ...
    for ep in range(epoch):
        ...
        #serializers.save_npz("%d_loss%.3f.npz" % (ep+1, sum_loss), model, compression=True)
        np.save("loss_history.npz", np.array(loss_history))
        unet.save("%d_loss%.3f"%(ep+1, sum_loss))
